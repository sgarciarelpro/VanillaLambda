            ...
            if (response.body.success) {
                ...
                if (event.headers.userToken != undefined && event.headers.userToken != "") {
                    var cardsSignUpOptions = {
                        host: host,
                        port: 443,
                        path: basePath + '/prospector/v1/auth/contact',
                        method: 'POST',
                        headers: {
                    ...
                    var errorResponse = createErrorMsg(-400, "contactSignUp");
                    console.log("Error=" + errorResponse);
                    return cb(null, {"statusCode": 400, "body":JSON.stringify(errResponse)});
                }

            } else {
                var errorResponse = createErrorMsg(-118, "contactSignUp");
                console.log("Error=" + errorResponse);
                return cb(null, {"statusCode": 400, "body":JSON.stringify(errResponse)});
            }
        });
    } else {
        var errorResponse = createErrorMsg(-110, "contactSignUp");
        console.log("Error=" + errorResponse);
        return cb(null, {"statusCode": 400, "body":JSON.stringify(errResponse)});
    }
};
